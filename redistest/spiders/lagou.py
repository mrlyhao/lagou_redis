...
from redistest.items import LagouJobItemLoader,LagouJobItem
from redistest.utils.commmon import get_md5
...
class LagouSpider(RedisCrawlSpider):
    ...
    # 爬取规则
    rules = (
        Rule(LinkExtractor(allow=('zhaopin/.*',)),follow = True),#跟踪zhaoping.下边的所有网站
        Rule(LinkExtractor(allow=('gongsi/j\d+.html',)), follow=True),
        Rule(LinkExtractor(allow=r'jobs/\d+.html'), callback="parse_job",follow=True),#符合本规则和requests交给parse_job处理
    )
